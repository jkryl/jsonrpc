...
mod tests {
	...
	use jsonrpc_core::{IoHandler, Params, Value, Error, ErrorCode};
	...
	fn should_call_one() {
		...
		let builder = ServerBuilder::new(io)
			.event_loop_executor(rt.executor());
		let server = builder.start(sock_path).expect("Couldn't open socket");
		let fut = connect(sock_path)
			.and_then(|client: RawClient| {
				let mut map = Map::new();
	...
	fn should_fail_without_server() {
		let mut rt = Runtime::new().unwrap();
		let fut = connect("/tmp/json-ipc-test.ipc")
			.and_then(|client: RawClient| {
				let mut map = Map::new();
	...
	fn should_handle_server_error() {
		let sock_path = "/tmp/json-ipc-test.ipc";
		let mut io = IoHandler::new();
		io.add_method("greeting", |_params| {
			Err(Error::invalid_params("test error"))
		});
		let mut rt = Runtime::new().unwrap();
		let builder = ServerBuilder::new(io)
			.event_loop_executor(rt.executor());
